...
function updatePromptPlaceholder() {
  el.promptInput.placeholder = state.selectedConversationId ? "Type a follow-up..." : "Ask anything.";
}
...
function buildActivitySnapshot({ taskId = "", status = "", events = [], summaries = [] } = {}) {
  ...
  const completed = Array.isArray(latestSummary?.completed) ? latestSummary.completed : [];
  const next = Array.isArray(latestSummary?.next) ? latestSummary.next : [];
  ...
  const summary = String(latestSummary?.summary || "").trim() || latestLine || summaryByStatus[normalizedStatus] || "Tracking workflow progress.";
  ...
  if (completed.length) {
    details.push(...completed.slice(0, 6).map((line) => `- Completed: ${String(line || "").trim()}`));
  }
  if (next.length) {
    details.push(...next.slice(0, 4).map((line) => `- Next: ${String(line || "").trim()}`));
  }
  if (safeEvents.length) {
    details.push("", "### Recent Events");
  ...
  return `
    ...
    >
      <div class="activity-head">
        ...
        <span class="activity-status">${escapeHtml(getStatusLabel(status))}</span>
      </div>
...
function patchActivityCardElement(card, snapshot) {
  ...
  if (statusEl) statusEl.textContent = getStatusLabel(snapshot.status);
...
function bindEvents() {
  ...
  el.messages.addEventListener("click", (event) => {
    ...
    if (startBtn) {
      ...
      if (!taskId) return;
      submitContinue(taskId).catch((err) => setNotice(`Start failed: ${err.message}`, true));
      return;
    }
