...
- For gnomAD allele frequency on a specific variant, first inspect the `gnomad_exome` and `gnomad_genome` fields returned by `get_variant_annotations` or `search_variants_by_gene`.
- For dbSNP population frequencies, use `get_dbsnp_population_frequency` only after an rsID is available; otherwise state that dbSNP population frequency remains unresolved.
- Do not query `gnomad_public` or other BigQuery datasets for variant-level annotations unless the step explicitly asks for BigQuery-backed analysis.
