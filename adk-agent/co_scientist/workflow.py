...
2. Organize the plan by evidence dimension and source capability. For comparisons, one step should apply the same source or tool family across all named entities so the executor can collect comparable fields with multiple calls inside that step.
3. Separate steps only when they require a different evidence source, a distinct dependency or handoff, or a materially different completion criterion. Treat discovery plus inspection of the records it returns as one evidence step when both serve the same criterion.
4. Select tools by their documented inputs, output scope, and evidence type. Prefer direct structured sources for measurable facts and literature for scientific interpretation or corroboration. Do not create one step per compared entity, repeat the same source as separate entity-specific steps, or add a report-writing step.
